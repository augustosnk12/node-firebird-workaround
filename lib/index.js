...
const // Protocol 10 includes support for warnings and removes the requirement for
...
const ptype_rpc = 2, // Simple remote procedure call
...
const // AUTH_PLUGIN_LIST = [AUTH_PLUGIN_SRP256, AUTH_PLUGIN_SRP, AUTH_PLUGIN_LEGACY],
...
exports.escape = function (value, protocolVersion) {
	...
	switch (typeof value) {
		case 'boolean':
			if ((protocolVersion || PROTOCOL_VERSION13) >= PROTOCOL_VERSION13) return value ? 'true' : 'false';
	...
	if (value instanceof Date)
		return (
			...
			value.getFullYear() +
			'-' +
			(value.getMonth() + 1).toString().padLeft(2, '0') +
			'-' +
			value.getDate().toString().padLeft(2, '0') +
			' ' +
			value.getHours().toString().padLeft(2, '0') +
			':' +
			value.getMinutes().toString().padLeft(2, '0') +
			':' +
			value.getSeconds().toString().padLeft(2, '0') +
			'.' +
			value.getMilliseconds().toString().padLeft(3, '0') +
			"'"
		);
				...
				switch (statement.type) {
					case isc_info_sql_stmt_select:
						statement.fetchAll(self, function (err, r) {
							...
							if (callback) callback(undefined, r, statement.output, true);
						});
					...
					case isc_info_sql_stmt_exec_procedure:
						if (ret && ret.data && ret.data.length > 0) {
							...
							if (callback) callback(undefined, ret.data[0], statement.output, true);
							...
							statement.fetch(self, 1, function (err, ret) {
								...
								if (callback) callback(undefined, ret.data[0], statement.output, false);
							});
	...
	self.connection.startTransaction(function (err, transaction) {
		...
		transaction.execute(
			...
			function (err, result, meta, isSelect) {
				...
				transaction.commit(function (err) {
					if (callback) callback(err, result, meta, isSelect);
				});
			},
			custom,
		);
...
exports.attach = function (options, callback) {
	...
	var cnx = (this.connection = new Connection(
		host,
		port,
		function (err) {
			if (err) {
			...
			cnx.connect(options, function (err) {
				if (err) {
					...
					if (manager) cnx.svcattach(options, callback);
					else cnx.attach(options, callback);
				}
			});
		},
		options,
		...
		function (err) {
			...
			cnx.connect(options, function (err) {
				...
				cnx.attach(options, function (err, ret) {
					if (!err) {
						if (self.db) self.db.emit('connect', ret);
...
ServiceManager.prototype._createOutputStream = function (optread, buffersize, callback) {
	...
	t.__proto__._read = function () {
		...
		fct.call(self, { buffersize: buffersize }, function (err, data) {
			...
			if (data.line && data.line.length) selfread.push(data.line, DEFAULT_ENCODING);
			else selfread.push(null);
		});
	};
...
ServiceManager.prototype._processcapabilities = function (blr, res) {
	...
	for (var i = 0; i < capArray.length; ++i) if (caps & (1 << i)) dbcapa.push(capArray[i]);
};
	...
	for (; tinfo !== isc_info_end; tinfo = br.readByteCode()) {
		switch (tinfo) {
			...
			case isc_info_svc_limbo_trans:
				// not implemented
				for (; tinfo !== isc_info_flag_end; tinfo = br.readByteCode()) break;
...
ServiceManager.prototype.backup = function (options, callback) {
	...
	if (factor) blr.addByteInt32(isc_spb_bkp_factor, factor);
	...
	if (opts) blr.addByteInt32(isc_spb_options, opts);
	if (verbose) blr.addByte(isc_spb_verbose);
	var self = this;
...
ServiceManager.prototype.restore = function (options, callback) {
	...
	if (readonly) blr.addByte(isc_spb_prp_am_readonly);
	else blr.addByte(isc_spb_prp_am_readwrite);
...
ServiceManager.prototype.getFbserverInfos = function (infos, options, callback) {
	...
	var tops = [],
		empty = isEmpty(infos);
	for (popts in opts) if (empty || infos[popts]) tops.push(opts[popts]);
...
Pool.prototype.check = function () {
	var self = this;
	if (self.dbinuse >= self.max) return self;
		...
		exports.attach(self.options, function (err, db) {
			if (!err) {
				...
				db.on('detach', function () {
					...
					if (self.pooldb.indexOf(db) !== -1 || self.internaldb.indexOf(db) === -1) return;
					...
					else self.pooldb.push(db);

					if (db.connection._pooled) self.dbinuse--;
					self.check();
				});
			} else {
...
exports.Connection.prototype._bind_events = function (host, port, callback) {
	...
	self._socket.on('error', function (e) {
		...
		if (self.db) self.db.emit('error', e);
				...
				if (r === op_cond_accept || r === op_accept_data) {
					...
					if (is_authenticated === 0) {
						if (cnx.options.pluginName && cnx.options.pluginName !== accept.pluginName) {
							doError(
								new Error(
									"Server don't accept plugin : " + cnx.options.pluginName + ', but support : ' + accept.pluginName,
								),
								callback,
							);
						}
...
Connection.prototype._queueEvent = function (callback) {
	...
	if (self._isClosed) {
		if (callback) callback(new Error('Connection is closed.'));
		return;
	}
...
Connection.prototype.connect = function (options, callback) {
	...
	function cb(err, ret) {
		...
		self.accept = ret;
		if (callback) callback(undefined, ret);
	}
...
Connection.prototype.attach = function (options, callback, db) {
	...
	if (role) blr.addString(isc_dpb_sql_role_name, role, DEFAULT_ENCODING);
	...
	blr.addString(
		...
		processName.length > 255 ? processName.substring(processName.length - 255, processName.length) : processName,
		DEFAULT_ENCODING,
	);
...
Connection.prototype.createDatabase = function (options, callback) {
	...
	function cb(err, ret) {
		if (ret) self.dbhandle = ret.handle;

		setImmediate(function () {
			if (self.db) self.db.emit('attach', ret);
		});
...
Connection.prototype.startTransaction = function (isolation, callback) {
	...
	if (this._isClosed) return this.throwClosed(callback);
...
Connection.prototype.commitRetaining = function (transaction, callback) {
	if (this._isClosed) throw new Error('Connection is closed.');
...
Connection.prototype.prepare = function (transaction, query, plan, callback) {
	...
	if (this.accept.protocolMinimumType === ptype_lazy_send) {
		// V11 Statement or higher
...
Connection.prototype.executeStatement = function (transaction, statement, params, callback, custom) {
	...
	function PrepareParams(params, input, callback) {
		...
		function putBlobData(index, value, callback) {
			self.createBlob2(transaction, function (err, blob) {
				...
				if (Buffer.isBuffer(value)) b = value;
				else if (typeof value === 'string') b = Buffer.from(value, DEFAULT_ENCODING);
				else if (!isStream) b = Buffer.from(JSON.stringify(value), DEFAULT_ENCODING);
		...
		for (var i = 0, length = params.length; i < length; i++) {
			...
			if (value === null || value === undefined) {
				...
				switch (meta.type) {
					...
					case SQL_TYPE_DATE:
					case SQL_TYPE_TIME:
						if (value instanceof Date) ret[i] = new SQLParamDate(value);
						else if (typeof value === 'string') ret[i] = new SQLParamDate(value.parseDate());
						else ret[i] = new SQLParamDate(new Date(value));
					...
					default:
						switch (typeof value) {
							case 'number':
								if (value % 1 === 0) {
									if (value >= MIN_INT && value <= MAX_INT) ret[i] = new SQLParamInt(value);
									else ret[i] = new SQLParamInt64(value);
								} else ret[i] = new SQLParamDouble(value);
								break;
							case 'string':
	...
	if (input.length) {
		...
		if (params.length !== input.length) {
			...
			callback(
				new Error(
					'Expected parameters: (params=' + params.length + ' vs. expected=' + input.length + ') - ' + statement.query,
				),
			);
			return;
		}
...
function bufferReader(buffer, max, writer, cb, beg, end) {
	...
	if (end >= buffer.length) end = undefined;
